...
// h1 stays the page's single top-level heading. The section stays a Server
// Component; <Reveal> (a client leaf) injects the scroll-in choreography without
// promoting this file to the client.
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        <Reveal className="mb-12 flex flex-col gap-6 md:mb-20">
          <div
        ...
        <Reveal as="ul" className="border-t border-border/70">
          {items.map((item, i) => (
            <li
              key={item.slug}
              className="reveal-item border-b border-border/70"
              style={revealIndex(i)}
            >
              <Link
                href={`/work/${item.slug}`}
